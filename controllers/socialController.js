...
exports.fetchFriends = async (req, res) => {
  ...
  try {
    ...
    if (!friendships.length) {
      return res.status(200).json([]); // Return an empty array if no friendships found
    }
    ...
    const friends = await User.findAll({
      where: {
        uid: { [Op.in]: friendUids.length ? friendUids : ["0"] }, // Prevent empty `Op.in` query
      },
      attributes: ["uid", "displayName", "photoUrl"], // Fetch necessary attributes
    });

    res.status(200).json(friends.length ? friends : []); // Ensure an array is always returned
  } catch (error) {
